          ...
          <ClientSideRoute key={post._id} route={`/post/${post.slug.current}`}>
            <div className="flex flex-col cursor-pointer group">
                ...
                <div className="absolute bottom-0 flex justify-between w-full p-5 text-white bg-black rounded bg-opacity-20 backdrop-blur-lg drop-shadow-lg">
                  ...
                  {post.categories && 
                    ...
                    {post.categories.map((category, i) => (
                      <div key={i} className="bg-[#5703d6] text-center text-white px-3 py-1 rounded-full text-sm font-semibold">
                        <p>
                          {category.title}
                        </p>
                      </div>
              ...
              <div className="flex-1 mt-5">
                ...
                <p className="text-gray-500 line-clamp-2">{post.short_text}</p>
              </div>
